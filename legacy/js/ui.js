...
class UI {
    ...
    async updateCartBadge() {
        const count = await this.db.getCartItemCount(); // Async
        ...
        if (stickyBar && stickyCount && stickyTotal) {
            ...
            if (count > 0 && isMarketplaceVisible) {
                ...
                const total = await this.db.getCartTotal(); // Async
                stickyTotal.textContent = Utils.formatPrice(total);
            } else {
                // Hide Bar
    ...
    async showMarketplace() {
        ...
        await this.updateCartBadge(); // Refresh Sticky Bar visibility
     ...
     */
    async renderMarketplace(category = 'all') {
        const container = Utils.$('marketplaceProducts');
        let products = await this.db.getProducts(); // Async
    ...
    async renderDashboard() {
        ...
        if (user.role === 'customer') {
            if (customerView) {
                ...
                await this._renderCustomerDashboard(user);
            }
        }
        // 3. Admin/Seller View logic
            ...
            if (storeView) {
                Utils.removeClass(storeView, 'hidden');
                await this._renderStoreDashboard(user);
            }
        }
    }
    ...
    async _renderStoreDashboard(user) {
        const storeView = Utils.$('storeDashboardView');
        if (!storeView) return;
        ...
        storeView.innerHTML = `
            ...
            <div id="storeContent" class="store-content">
                <!-- Dynamic Content -->
                <div style="padding: 40px; text-align: center;"><span class="loader"></span></div>
            </div>
        `;

        await this._renderStoreContent(user);
    }

    async _renderStoreContent(user) {
        const container = Utils.$('storeContent');
        if (!container) return;

        if (this.currentStoreTab === 'products') {
            await this._renderStoreProducts(user, container);
        } else if (this.currentStoreTab === 'users') {
            await this._renderStoreUsers(user, container);
        } else if (this.currentStoreTab === 'settings') {
            this._renderStoreSettings(user, container);
        } else if (this.currentStoreTab === 'sellers') {
            await this._renderAdminSellers(user, container);
            ...
            await this._renderSellerMonitor(sellerId, container);
        }
    }

    async _renderAdminSellers(user, container) {
    ...
    async _renderSellerMonitor(sellerId, container) {
        const allUsers = await this.db.getAllUsers();
        const seller = allUsers.find(u => u.id === sellerId);
        if (!seller) return;

        // Reuse User List Logic but specific to this seller's customers
        const customers = await this.db.getCustomersBySeller(seller.username); // Assuming logic uses username matcher in DB
    ...
    async _renderStoreProducts(user, container) {
        ...
        await this._renderStoreProductGrid(user);
    }

    async _renderStoreProductGrid(user) {
        const grid = document.getElementById('dashboardProducts');
        if (!grid) return;

        let products = await this.db.getUserProducts();
    ...
    async _renderStoreUsers(user, container) {
        let users;
        // Filter: Seller only sees Customers who bought from them
        if (user.role === 'seller') {
            users = await this.db.getCustomersBySeller(user.id);
        } else {
    ...
    updateImagePreview(base64) {
        ...
        if (container) {
            ...
            container.innerHTML = `
            <div style="position: relative; width: 100%; height: 100%;" class="group">
    ...
    async searchProducts() {
        ...
        // Show loading
        container.innerHTML = '<div style="padding: 40px; text-align: center;"><span class="loader"></span></div>';

        // Use getProducts safely since searchProducts might not exist in DB yet
        // or implement search on backend. For MVP, filter client side from getProducts
        // or add searchProducts to DB.
        // Let's assume we use getProducts() and filter for now as per previous logic
        // But wait, the previous code called this.db.searchProducts(query).
        // I need to make sure that method exists and is async.

        let products = await this.db.searchProducts(query);
